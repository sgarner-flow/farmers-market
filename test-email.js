// We'll use a simpler approach - define our own template function directly
function createSimpleEmailTemplate(options) {
...
async function sendTestEmail() {
  try {
    ...
    const emailContent = createSimpleEmailTemplate({
      title: 'Test Email',
      previewText: 'This is a test email from Flow Farmers Market',
      mainContent: `
        <h2 style="color: #4A4A4A; margin-bottom: 20px;">Hello from Flow Farmers Market!</h2>
        <p style="margin-bottom: 20px;">This is a test email to verify that our email templates are working correctly.</p>
        <p style="margin-bottom: 20px;">The email template is rendering as expected.</p>
    ...
    // Configure email
    const msg = {
      to: 'sgarns@gmail.com', // Your email address
      from: {
